...
def get_search_strings(from_date, to_date, episode_db):
    found_episodes = []
    searches_to_perform = []
    ...
    for search in searches_to_perform:
        ...
        for search_result in search_results:
            if output_directory is not None and os.path.isdir(output_directory):
                magnet_file_path = os.path.join(output_directory, search_result.title + ".magnet")
...
def find_downloads(args, episode_db):
...
def convert(args, episode_db, string_substitutions=[], ffmpeg_location=None):
    series_metadata = episode_db.get_tracked_series_by_keyword(args.keyword)
    if args.keyword is not None and series_metadata is None:
        ...
        if args.dry_run:
            for src_file, dest_file in file_map:
                converted_dest_file = replace_strings(dest_file, string_substitutions)
                print("Convert {} -> {}".format(src_file, converted_dest_file))

        for src_file, dest_file in file_map:
            converted_dest_file = replace_strings(dest_file, string_substitutions)
            converter = Converter(src_file, converted_dest_file, ffmpeg_location)
